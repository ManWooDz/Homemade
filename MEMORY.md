...
**Allergy check only catches explicit allergen-word mentions, not known derived ingredients (2026-09-19, unfixed):**
...
- root cause: `check_schema_and_constraints`'s allergy check (`backend/eval/metrics.py`) is a literal substring match against the stated allergen word only (e.g. `"กุ้ง"`). It has no lookup mapping a known derived-ingredient name (`กะปิ`, `น้ำปลา` made with shrimp, etc.) to the allergen it's actually derived from — so any generator output that names the ingredient without ever spelling out the allergen word passes the check undetected, even though it's the same real-world violation.
- correct: not fixed yet. The fix is a small known-derived-ingredient → allergen lookup table added to the allergy check (e.g. `กะปิ` → `กุ้ง`), not a Unicode-normalization fix — this is a different, more serious gap than the normalization issue logged below (that one is a false positive on a safe ingredient; this one is a false negative on an actual allergen). Do not conflate the two when revisiting this metric.
